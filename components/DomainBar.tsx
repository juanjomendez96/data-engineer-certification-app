    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between text-sm">
        <span className="text-slate-200 font-medium">
          {displayName}
          <span className="text-slate-400 ml-1 font-normal">({Math.round(weight * 100)}%)</span>
        </span>
        <span className={cn('font-semibold tabular-nums', passed ? 'text-green-400' : 'text-red-400')}>
      ...
      <div className="relative h-3 rounded-full bg-slate-700 overflow-hidden">
        ...
        />
        {/* 70% threshold marker */}
        <div className="absolute top-0 bottom-0 w-0.5 bg-white opacity-70" style={{ left: '70%' }} />
      </div>
    </div>
  );
}
